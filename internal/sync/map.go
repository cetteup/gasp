...
// This is just a copy of sync/map updated to work with generics

// Map is like a Go map[comparable]comparable but is safe for concurrent use
...
// [Basket.Load], [Map.LoadAndDelete], [Map.LoadOrStore], [Basket.Swap], [Map.CompareAndSwap],
// and [Map.CompareAndDelete] are read operations;
// [Map.Delete], [Basket.LoadAndDelete], [Basket.Store], and [Map.Swap] are write operations;
...
// the entry unchanged.
func (e *entry[T]) tryCompareAndSwap(old, fresh T) bool {
	p := e.p.Load()
	if p == nil || p == new(T) || *p != old {
		return false
	}
...
func (e *entry[T]) unexpungeLocked() (wasExpunged bool) {
	return e.p.CompareAndSwap(new(T), nil)
}
...
func (m *Map[K, V]) CompareAndSwap(key K, old, fresh V) (swapped bool) {
	read := m.loadReadOnly()
	if e, ok := read.m[key]; ok {
		return e.tryCompareAndSwap(old, fresh)
	...
	swapped = false
	if e, ok := read.m[key]; ok {
		swapped = e.tryCompareAndSwap(old, fresh)
	} else if e, ok := m.dirty[key]; ok {
		swapped = e.tryCompareAndSwap(old, fresh)
...
func (e *entry[T]) tryExpungeLocked() (isExpunged bool) {
	p := e.p.Load()
	for p == nil {
		if e.p.CompareAndSwap(nil, new(T)) {
			return true
		}
		p = e.p.Load()
	}
